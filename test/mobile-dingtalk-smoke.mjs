...
try {
  ...
  await page.goto('http://127.0.0.1:39904/', { waitUntil: 'domcontentloaded' });
  await page.locator('#app:not([hidden])').waitFor({ timeout: 15_000 });
  const entryCount = await page.locator('button[data-tab="dingtalk"]').count();
  if (entryCount !== 0) throw new Error(`钉钉入口未隐藏：${entryCount}`);
  const viewPresent = await page.evaluate(() => Boolean(document.querySelector('#dingtalkView')));
  const viewActive = await page.evaluate(() => document.querySelector('#dingtalkView')?.classList.contains('active'));
  if (!viewPresent || viewActive) throw new Error(`钉钉视图状态异常：present=${viewPresent} active=${viewActive}`);

  process.stdout.write(`${JSON.stringify({ entryHidden: true, viewPresent, viewActive: false })}\n`);
} finally {
  await browser.close();
}
