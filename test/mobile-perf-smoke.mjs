...
try {
  ...
  const moreText = await page.locator('.artifact-more').textContent();
  if (!moreText.includes('940')) throw new Error(`显示更多文案错误：${moreText}`);

  // 2) 显示更多每次追加 100 条
  const renderMoreStart = Date.now();
  await page.locator('.artifact-more').click();
  await page.waitForTimeout(150);
  const afterMoreCards = await page.locator('.artifact-card').count();
  const moreRenderMs = Date.now() - renderMoreStart;
  if (afterMoreCards !== 160) throw new Error(`显示更多后应 160 张卡片，实际 ${afterMoreCards}`);
  ...
  process.stdout.write(`${JSON.stringify({ initialCards, moreText, afterMoreCards, switchToChatMs: +switchToChatMs.toFixed(1), moreRenderMs, roundTripMs: +roundTripMs.toFixed(1) })}\n`);
} finally {
  await browser.close();
}
