...
try {
  ...
  const pageErrors = [];
  page.on('console', (msg) => {
    if (msg.type() === 'debug' && msg.text().includes('[codex-mobile]')) debugLogs.push(msg.text());
  });
  ...
  await page.goto('http://127.0.0.1:39905/?debug=1', { waitUntil: 'domcontentloaded' });
  await page.locator('#app:not([hidden])').waitFor({ timeout: 15_000 });
  await page.waitForTimeout(500);
  if (pageErrors.length) throw new Error(`debug 页面出现错误：${pageErrors.join('; ')}`);
  if (!debugLogs.length) throw new Error('debug=1 未输出诊断日志');

  process.stdout.write(`${JSON.stringify({ debugLogs: debugLogs.length, sample: debugLogs[0], pageErrors: 0 })}\n`);
} finally {
  await browser.close();
}
